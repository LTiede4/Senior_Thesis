...
class ConfigurationLoader(object):
    ...
    def __init__(self, config: str | Path | dict) -> None:
        ...
        # Assume that the configuration is a dictionary with all params
            ...
            msg = (
                "Keyword 'config' should provide the name of the file (string or pathlib.Path)"
...
class MultiAfgen(object):
    ...
    def _check_x_ascending(self, tbl_xyz: list) -> tuple[list, list]:
        ...
        if sum_break == 0:
            ...
            msg = "X values for AFGEN input list not strictly ascending: %s" % z_list
            raise ValueError(msg)
...
def astro(day: dt.date, latitude: float, radiation: float, _cache: dict = {}) -> astro_nt:
    ...
    DEC = -asin(sin(23.45 * RAD) * cos(2.0 * pi * (float(IDAY) + 10.0) / 365.0))
    SC = 1370.0 * (1.0 + 0.033 * cos(2.0 * pi * float(IDAY) / 365.0))
    ...
    if abs(AOB) <= 1.0:
        ...
        DSINB = 3600.0 * (DAYL * SINLD + 24.0 * COSLD * sqrt(1.0 - AOB**2) / pi)
        DSINBE = 3600.0 * (
            DAYL * (SINLD + 0.4 * (SINLD**2 + COSLD**2 * 0.5))
            + 12.0 * COSLD * (2.0 + 3.0 * 0.4 * SINLD) * sqrt(1.0 - AOB**2) / pi
        )
    else:
        if AOB > 1.0:
            DAYL = 24.0
        ...
        DSINBE = 3600.0 * (DAYL * (SINLD + 0.4 * (SINLD**2 + COSLD**2 * 0.5)))
...
def daylength(day: dt.date, latitude: float, angle: float = -4, _cache: dict = {}) -> float:
    ...
    # daylength
    if abs(AOB) <= 1.0:
        DAYLP = 12.0 * (1.0 + 2.0 * asin((-sin(ANGLE * RAD) + SINLD) / COSLD) / pi)
    elif AOB > 1.0:
        DAYLP = 24.0
    else:
        DAYLP = 0.0
...
def reference_ET(
    ...
    **kwargs: dict,
    ...
    E0, ES0, ET0 = penman(DAY, LAT, ELEV, TMIN, TMAX, IRRAD, VAP, WIND, ANGSTA, ANGSTB)
    if ETMODEL == "PM":
...
def penman(
    ...
    TMPA = (TMIN + TMAX) / 2.0
    TDIF = TMAX - TMIN
    BU = 0.54 + 0.35 * limit(0.0, 1.0, (TDIF - 12.0) / 4.0)

    # barometric pressure (mbar)
    # psychrometric constant (mbar/Celsius)
    PBAR = 1013.0 * exp(-0.034 * ELEV / (TMPA + 273.0))
    GAMMA = PSYCON * PBAR / 1013.0
    ...
    RELSSD = limit(0.0, 1.0, (r.ATMTR - abs(ANGSTA)) / abs(ANGSTB))
    ...
    RNW = (AVRAD * (1.0 - REFCFW) - RB) / LHVAP
    RNS = (AVRAD * (1.0 - REFCFS) - RB) / LHVAP
    RNC = (AVRAD * (1.0 - REFCFC) - RB) / LHVAP

    # evaporative demand of the atmosphere (mm/d)
    EA = 0.26 * max(0.0, (SVAP - VAP)) * (0.5 + BU * WIND2)
    EAC = 0.26 * max(0.0, (SVAP - VAP)) * (1.0 + BU * WIND2)
...
def penman_monteith(
    ...
    DELTA = (4098.0 * SVAP_TMPA) / pow((TMPA + 237.3), 2)
    ...
    RNL_TMP = ((STB_TMAX + STB_TMIN) / 2.0) * (0.34 - 0.14 * sqrt(VAP))
    ...
    if CSKYRAD > 0:
        ...
        EA = (900.0 / (TMPA + 273)) * WIND2 * (SVAP - VAP)

        # Modified psychometric constant (gamma*)[kPa/C]
        MGAMMA = GAMMA * (1.0 + (CRES / 208.0 * WIND2))
