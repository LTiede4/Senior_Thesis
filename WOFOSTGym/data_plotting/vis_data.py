...
def plot_output(
    args: utils.Args,
    output_vars: np.ndarray | list = None,
    obs: np.ndarray = None,
    ...
    **kwargs,
    ...
    for i, v in enumerate(output_vars):
        ...
        if save:
            ...
            ), f"Folder args.fig_folder `{args.save_folder}` must be of type `str`"
        ...
        if hasattr(args, "fig_folder"):
            if args.fig_folder is not None:
                assert isinstance(
                    args.fig_folder, str
                ), f"Folder args.fig_folder `{args.fig_folder}` must be of type `str`"
...
def plot_policy(
    ...
    **kwargs,
    ...
    n = [
        [
            patches.Rectangle(
                (totn_inds[i], 0), 1, totn_vals[i], facecolor=("g", 0.5), edgecolor=("k", 0.7), hatch=utils.HATCHES[0]
            )
    ...
    p = [
        [
            patches.Rectangle(
                (totp_inds[i], 0), 1, totp_vals[i], facecolor=("m", 0.5), edgecolor=("k", 0.7), hatch=utils.HATCHES[0]
            )
            for i in range(len(totp_inds))
        ]
        for j in range(len(totn_inds))
    ]
    [[ax.add_patch(pi) for pi in pj] for pj in p]
    k = [
        [
            patches.Rectangle(
                (totk_inds[i], 0), 1, totk_vals[i], facecolor=("y", 0.5), edgecolor=("k", 0.7), hatch=utils.HATCHES[0]
            )
            for i in range(len(totk_inds))
        ]
        for j in range(len(totn_inds))
    ]
    ...
    w = [
        [
            patches.Rectangle(
                ...
                facecolor=("b", 0.5),
                edgecolor=("k", 0.7),
                hatch=utils.HATCHES[0],
            )
            for i in range(len(totirrig_inds))
        ]
        for j in range(len(totn_inds))
    ]
    [[ax.add_patch(wi) for wi in wj] for wj in w]

    n = patches.Patch(color="g", alpha=0.6, label="Nitrogen")
    p = patches.Patch(color="m", alpha=0.6, label="Phosphorous")
    k = patches.Patch(color="y", alpha=0.6, label="Potassium")
    w = patches.Patch(color="b", alpha=0.6, label="Water")
    hands = [n, p, k, w, wso[0]]
...
def plot_policy_multiple(
    ...
    **kwargs,
    ...
    labs = [
        twinax.plot(
            split_obs[i][:, np.argwhere(output_vars == twinax_varname).flatten()[0]],
            label=f"Policy {i}: {twinax_varname}",
        )
        for i in range(len(split_obs))
    ]
    ...
    n = [
        [
            patches.Rectangle(
                ...
                1,
                totn_vals[j][i],
                hatch=utils.HATCHES[j % len(utils.HATCHES)],
                #edgecolor=(utils.COLORS[j], 0.5),
                edgecolor=(utils.COLORS[j % len(utils.COLORS)], 0.5),
                facecolor=("g", 0.5),
            )
            for i in range(len(totn_inds[j]))
        ]
        for j in range(len(totn_inds))
    ]
    [[ax.add_patch(ni) for ni in nj] for nj in n]
    ...
    p = [
        [
            ...
            for i in range(len(totp_inds[j]))
        ]
        for j in range(len(totp_inds))
    ]
    [[ax.add_patch(pi) for pi in pj] for pj in p]
    ...
    k = [
        [
            ...
            for i in range(len(totk_inds[j]))
        ]
        for j in range(len(totk_inds))
    ]
    [[ax.add_patch(ki) for ki in kj] for kj in k]
    ...
    w = [
        [
            ...
            for i in range(len(totirrig_inds[j]))
        ]
        for j in range(len(totirrig_inds))
    ]
