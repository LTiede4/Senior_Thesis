...
class Base_WOFOST_Root_Dynamics(SimulationObject):
    ...
    **External dependencies:**
    ...
    class RateVariables(RatesTemplate):
        RR = Float(-99.0)
        GRRT = Float(-99.0)
        DRRT1 = Float(-99.0)  # Death rate of roots due to aging
        DRRT2 = Float(-99.0)  # Death rate of roots due to excess water
        DRRT3 = Float(-99.0)  # Death rate of roots due to fertilizer burn
        DRRT = Float(-99.0)
        GWRT = Float(-99.0)
...
class Annual_WOFOST_Root_Dynamics(Base_WOFOST_Root_Dynamics):
    ...
    def initialize(self, day: date, kiosk: VariableKiosk, parvalues: dict) -> None:
        ...
        self.states = self.StateVariables(
            kiosk, publish=["RD", "RDM", "WRT", "DWRT", "TWRT"], RD=RD, RDM=RDM, WRT=WRT, DWRT=DWRT, TWRT=TWRT
        )
