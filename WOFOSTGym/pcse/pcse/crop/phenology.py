...
class Vernalisation(SimulationObject):
    ...
    **External dependencies:**
    ...
    def initialize(self, day: datetime.date, kiosk: VariableKiosk, parvalues: dict) -> None:
        ...
        self.states = self.StateVariables(kiosk, VERN=0.0, ISVERNALISED=False, publish=["VERN", "ISVERNALISED"])
...
class Base_Phenology(SimulationObject):
    ...
    **Signals sent or handled**
    ...
    class Parameters(ParamTemplate):
        ...
        TSUM3 = Float(-99.0)  # Temperature sum from maturity to death
        ...
        DVSM = Float(-99.0)  # Mature development stage
    ...
    class RateVariables(RatesTemplate):
        ...
        RDEM = Int(-99.0)  # Days above temp sum
    ...
    def _get_initial_stage(self, day: datetime.date) -> tuple[int, datetime.date, str]:
        ...
        if p.CROP_START_TYPE == "emergence":
            ...
            msg = (
                "Unknown start type: %s. Are you using the corect Phenology \
                module (Calling the correct Gym Environment)?"
                % p.CROP_START_TYPE
            )
    ...
    def calc_rates(self, day: datetime.date, drv: WeatherDataContainer) -> None:
        ...
        if p.IDSL >= 1:
            ...
            DVRED = limit(0.0, 1.0, (DAYLP - p.DLC) / (p.DLO - p.DLC))
        ...
        if s.STAGE == "sowing":
            ...
            r.DTSUME = limit(0.0, (p.TEFFMX - p.TBASEM), (drv.TEMP - p.TBASEM))
            r.DTSUM = 0.0
    ...
    def _next_stage(self, day: datetime.date) -> None:
        ...
        if s.STAGE == "sowing":
            ...
            if p.CROP_END_TYPE in ["emergence"]:
                self._send_signal(signal=signals.crop_finish, day=day, finish_type="emergence", crop_delete=True)
        elif s.STAGE == "vegetative":
            ...
            if p.CROP_END_TYPE in ["maturity"]:
                self._send_signal(signal=signals.crop_finish, day=day, finish_type="maturity", crop_delete=True)
        elif s.STAGE == "mature":
            s.STAGE = "dead"
            if p.CROP_END_TYPE in ["death"]:
                self._send_signal(signal=signals.crop_finish, day=day, finish_type="death", crop_delete=True)
        elif s.STAGE == "dead":
...
class Annual_Phenology(Base_Phenology):
    ...
    def initialize(self, day: datetime.date, kiosk: VariableKiosk, parvalues: dict) -> None:
        ...
        self.states = self.StateVariables(
            kiosk,
            publish=["DVS", "TSUM", "TSUME", "STAGE", "DOP", "DATBE"],
            TSUM=0.0,
            TSUME=0.0,
            DOP=DOP,
            DVS=DVS,
...
class Perennial_Phenology(Base_Phenology):
    ...
    **Rate variables**
    ...
    class Parameters(ParamTemplate):
        ...
        MLDORM = Float(-99.0)  # Daylength at which a plant will go into dormancy
    ...
    def initialize(self, day: datetime.date, kiosk: VariableKiosk, parvalues: dict) -> None:
        ...
        self.states = self.StateVariables(
            kiosk,
            publish=["DVS", "TSUM", "TSUME", "STAGE", "DSNG", "DSD", "AGE", "DCYCLE", "DATBE", "DOP"],
            TSUM=0.0,
            TSUME=0.0,
            DVS=DVS,
            STAGE=STAGE,
...
class Grape_Phenology(SimulationObject):
    ...
    **Signals sent or handled**
    ...
    _HC_YESTERDAY = Float(-99.0)  # Helper variable for yesterday's HC
    ...
    class Parameters(ParamTemplate):
        ...
        DVSM = Float(-99.0)  # Mature development stage
        DVSEND = Float(-99.0)  # Final development stage
        TBASEM = Float(-99.0)  # Base temp. for bud break
        TEFFMX = Float(-99.0)  # Max eff temperature for grow daily units
        TSUMEM = Float(-99.0)  # Temp. sum for bud break

        TSUM1 = Float(-99.0)  # Temperature sum budbreak to flowering
        TSUM2 = Float(-99.0)  # Temperature sum flowering to verasion
        TSUM3 = Float(-99.0)  # Temperature sum from verasion to ripe
        TSUM4 = Float(-99.0)  # Temperature sum from ripe onwards
        MLDORM = Float(-99.0)  # Daylength at which a plant will go into dormancy
        Q10C = Float(-99.0)  # Parameter for chilling unit accumulation
        CSUMDB = Float(-99.0)  # Chilling unit sum for dormancy break

        # Cold Hardiness parameters
        HCINIT = Float(-99.0)  # Initial Cold Hardiness
        HCMIN = Float(-99.0)  # Minimum cold hardiness (negative)
        HCMAX = Float(-99.0)  # Maximum cold hardiness (negative)
        TENDO = Float(-99.0)  # Endodorm temp
        TECO = Float(-99.0)  # Ecodorm temp
        ENACCLIM = Float(-99.0)  # Endo rate of acclimation
        ECACCLIM = Float(-99.0)  # Eco rate of acclimation
        ENDEACCLIM = Float(-99.0)  # Endo rate of deacclimation
        ECDEACCLIM = Float(-99.0)  # Eco rate of deacclimation
        THETA = Float(-99.0)  # Theta param for acclimation
    ...
    class RateVariables(RatesTemplate):
        ...
        DVR = Float(-99.0)  # development rate
        RDEM = Int(-99.0)  # Days above temp sum
        DCU = Float(-99.0)  # Daily chilling units

        # Cold hardiness rates
        DHR = Float(-99.0)  # Daily heating rate
        DCR = Float(-99.0)  # Daily chilling rate
        DACC = Float(-99.0)  # Deacclimation rate
        ACC = Float(-99.0)  # Acclimation rate
        HCR = Float(-99.0)  # Change in acclimation

    class StateVariables(StatesTemplate):
        PHENOLOGY = Int(-0.99)  # Int of Stage
        DVS = Float(-99.0)  # Development stage
        TSUM = Float(-99.0)  # Temperature sum state
        CSUM = Float(-99.0)  # Chilling sum state
        ...
        DHSUM = Float(-99.0)  # Daily heating sum
        DCSUM = Float(-99.0)  # Daily chilling sum
        HC = Float(-99.0)  # Cold hardiness
        PREDBB = Float(-99.0)  # Predicted bud break
        LTE50 = Float(-99.0)  # Predicted LTE50 for cold hardiness
    ...
    def initialize(self, day: datetime.date, kiosk: VariableKiosk, parvalues: dict) -> None:
        ...
        self.states = self.StateVariables(
            ...
            DOC=None,
            DON=None,
            PHENOLOGY=self._STAGE_VAL[STAGE],
            DHSUM=0.0,
            DCSUM=0.0,
            HC=p.HCINIT,
            PREDBB=0.0,
        ...
        self.rates = self.RateVariables(kiosk, publish=["DTSUME", "DTSUM", "DVR", "DHR", "DCR", "DACC", "ACC", "HCR"])
    ...
    def calc_rates(self, day: datetime.date, drv: WeatherDataContainer) -> None:
        ...
        if s.STAGE == "endodorm":
            ...
            r.DTSUM = max(0.0, daily_temp_units(drv, p.TBASEM, p.TEFFMX))
            r.DVR = r.DTSUM / (p.TSUM4)
            r.DHR = max(0.0, drv.TEMP - p.TENDO)
            r.DCR = min(0.0, drv.TEMP - p.TENDO)
            if s.DCSUM != 0:
            ...
            r.DTSUME = max(0.0, daily_temp_units(drv, p.TBASEM, p.TEFFMX))
            r.DVR = 0.1 * r.DTSUME / p.TSUMEM
            r.DHR = max(0.0, drv.TEMP - p.TECO)
            r.DCR = min(0.0, drv.TEMP - p.TECO)
            if s.DCSUM != 0:
