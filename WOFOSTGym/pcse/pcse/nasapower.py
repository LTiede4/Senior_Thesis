...
class NASAPowerWeatherDataProvider(WeatherDataProvider):
    ...
    def __init__(self, latitude: float, longitude: float, force_update: bool = False, ETmodel: str = "PM") -> None:
        ...
        if age < 90:
            ...
            if status is not True:
                ...
                self._get_and_process_NASAPower(self.latitude, self.longitude)
            # show which cache we are loading
            # command to remove washington cahce:
            # rm -f /home/ltiede/thesis/WOFOSTGym/pcse/pcse/.pcse/meteo_cache/NASAPowerWeatherDataProvider_LAT00470_LON-1200.cache
            # else:
            #     print(f"Loaded from cache: {cache_file}")
            #     print(f"Number of days in store: {len(self.store)}")
            #     if len(self.store) > 0:
            #         first_day = min(self.store.keys())[0]
            #         last_day = max(self.store.keys())[0]
    ...
    def _get_and_process_NASAPower(self, latitude: float, longitude: float) -> None:
        ...
        # debugging code
        #print(f"Successfully retrieved NASA Weather.")
        ...
        if len(self.store) > 0:
            ...
            # # Check if Jan 1, 1987 is in the store
            # if (dt.date(1987, 1, 1), 0) in self.store:
            #     print("Jan 1, 1987 is in the cache")
            # else:
            #     print("Jan 1, 1987 is NOT in the cache")
            #     # Print the first few dates to see what we have
            #     print("First 5 dates in store:")
            #     for i, (date, _) in enumerate(sorted(self.store.keys())[:5]):
            #         print(f"  {date}")
    ...
    def _query_NASAPower_server(self, latitude: float, longitude: float) -> str:
        ...
        # changed from current date, for out of bounds error
        end_date = dt.date(2023, 12, 31)
        #end_date = dt.date.today()

        # DEBUG: Print the parameters being sent
        #print(f"Parameters: {','.join(self.power_variables)}")
        ...
        url = requests.get(server, params=payload).url
        ...
        if req.status_code != self.HTTP_OK:
            ...
            #print(f"Response content: {req.text[:500]}")  # Print first 500 chars of response
            raise exc.PCSEError(msg)
