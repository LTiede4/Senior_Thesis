...
def eval_policy(
    ...
    # add bounds to ensure it stays within cached data
    new_args.year_low = 1983
    new_args.year_high = 2023
    ...
    for i in range(eval_episodes):
        ...
        while not (term or trunc):
            ...
            try:
                ...
                print(f"Warning: Missing weather data, ending episode early. Error: {e}")
                # End the episode
                term = True
                trunc = False
                reward = 0
...
def eval_policy_lstm(
    ...
    # Check if the environment expects a config parameter
    sig = inspect.signature(env_constr.__init__)
    if len(sig.parameters) == 10:  # 9 args + self = 10 total parameters
        env = env_constr(
            new_args,
            base_fpath,
            agro_fpath,
