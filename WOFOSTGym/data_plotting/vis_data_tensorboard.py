...
def load_scalars_from_runs(log_dirs: list = None, scalar_name: str = None) -> dict[str, np.ndarray]:
    """
...
def plot_scalars(data:dict[str, np.ndarray], names:list[str], args:PlotArgs, log_scale:bool=False) -> None:
    """
...
def plot_reward_log(data:dict[str,np.ndarray], names:list[str], args:PlotArgs) -> None:
    """
...
def plot_reward_figs_two(data:dict[str,np.ndarray], names:list[str], args:PlotArgs) -> None:
    """
    ...
    for i, d in enumerate(data.items()):
        log_dir, (steps, values) = d
        avg, std = running_average(values, window_size=200)
        if i > 3:
            formatter = ticker.ScalarFormatter(useMathText=True)
            formatter.set_powerlimits((4, 4))
            ax[1].yaxis.set_major_formatter(formatter)
    ...
    #ax[1].axhline(rews[1], color="black", linestyle="--", label="Grape Potential")
    ...
    ax[1].set_title("Maize Over One Season", fontsize=fnt_size)
...
def plot_reward_figs_one(data:dict[str,np.ndarray], names:list[str], args:PlotArgs) -> None:
    """
    Plot reward data on two side by side plots
    ...
    for i, d in enumerate(data.items()):
        ...
        if i > 3:
            ...
            ax.plot(steps, avg, label=names[i], color=utils.COLORS[i])
            ax.fill_between(steps, avg + std, avg - std, color=utils.COLORS[i], alpha=0.4)

    # ax[0].axhline(rews[0], color='black', linestyle='--', label='Potential')
    ax.axhline(rews[2], color="g", linestyle="--", label="BiWeekly NW")
    ax.axhline(rews[4], color="c", linestyle="--", label="Monthly W")
    #ax[1].axhline(rews[1], color="black", linestyle="--", label="Grape Potential")
    #ax[1].axhline(rews[3], color="g", linestyle="--", label="BiWeekly NW")
    #ax[1].axhline(rews[5], color="c", linestyle="--", label="Monthly W")
    ...
    #ax[1].set_xlim((0, 5000000))
    ax.set_title("Jujube Over Three Seasons", fontsize=fnt_size)
    ...
    ax.tick_params(axis="x", labelsize=lb_size)
    #ax[1].tick_params(axis="x", labelsize=lb_size)
    ax.set_xlabel("Episode Steps", fontsize=fnt_size)
    #ax[1].set_xlabel("Episode Steps", fontsize=fnt_size)
    ax.set_ylabel("Reward (Yield)", fontsize=fnt_size)
    #ax[1].set_ylabel("Reward (Yield)", fontsize=fnt_size)
    ax.legend(handlelength=1, ncol=2, fontsize=fnt_size)
    #ax[1].legend(handlelength=1, ncol=2, fontsize=fnt_size)
    plt.savefig(f"{args.save_name}_single.png", bbox_inches="tight")

def plot_grapevine_unconstrained(data:dict[str,np.ndarray], names:list[str], args:PlotArgs) -> None:
    """
    Plot reward data for grapevine unconstrained experiment
    """
    # These need to be replaced with your actual baseline values
    grape_potential = 725497  # From rews[1]
    biweekly_nw = 141584      # From rews[3]  
    monthly_w = 130114        # From rews[5]
    ...
    for i, d in enumerate(data.items()):
        ...
        ax.plot(steps, avg, label=names[i], color=utils.COLORS[i], linewidth=2)
        ax.fill_between(steps, avg + std, avg - std, color=utils.COLORS[i], alpha=0.3)

    # Add baselines
    ax.axhline(grape_potential, color="black", linestyle="--", linewidth=2, label="Grape Potential")
    ax.axhline(biweekly_nw, color="green", linestyle="--", linewidth=2, label="BiWeekly NW")
    ax.axhline(monthly_w, color="cyan", linestyle="--", linewidth=2, label="Monthly W")

    ax.set_xlim((0, 5000000))  # 5M steps for grapevine
    ax.set_title("Grapevine Over One Season", fontsize=fnt_size)
    ax.set_xlabel("Episode Steps", fontsize=fnt_size)
    ax.set_ylabel("Reward (Yield)", fontsize=fnt_size)
    ax.tick_params(axis="both", labelsize=lb_size)
    ax.legend(handlelength=1, ncol=2, fontsize=lb_size, loc="lower right")
    
    plt.tight_layout()
    plt.savefig(f"{args.save_name}.png", bbox_inches="tight", dpi=300)
    plt.show()
...
def running_average(arr, window_size=50):
    ...
    pad_right = max(0, len(arr) - len(running_avg) - half_window)
    # Ensure pad_right is not negative
    if pad_right < 0:
        pad_right = 0
    
    if len(running_avg) > 0:
        running_avg = np.pad(running_avg, (pad_left, pad_right), mode="edge")
        running_std = np.pad(running_std, (pad_left, pad_right), mode="edge")
    else:
        running_avg = arr.copy()
        running_std = np.zeros_like(arr)

    return running_avg, running_std


def wandb_plot_scalars(api_name: str= "solow/cropsim_times") -> None:
    """
    Plot Scalars from WandB"""
    api = wandb.Api()

    # Get all runs in a project
    runs = api.runs(api_name)
...
def plot_reward_log_runoffbar(data:dict[str,np.ndarray], names:list[str], args:PlotArgs) -> None:
    """
...
def load_bar_data(directories:list[str]=["experiments/data"], ag_type:str="RPPO") -> tuple[np.ndarray, np.ndarray]:
    """
...
def plot_reward_log_hist(data:dict[str,np.ndarray], names:list[str], args:PlotArgs) -> None:
    """
    Plot scalar data on a single matplotlib plot.
...
def load_hist_data(directories:list[str]=["experiments/runs"], prefixes:list[str]=["experiments/data/pear"]) -> np.ndarray:
    """
    Load np.ndarray of histogram data for nutrient application likelihoods
    """
    # Get all NPZ files
    npz_files = []
    for directory in directories:
...
def plot_scalars_default_yield(data:dict[str,np.ndarray], names:list[str], args:PlotArgs) -> None:
    """
    Plot scalar data on a single matplotlib plot.
...
def plot_default_yield(data:dict[str,np.ndarray], names:list[str], args:PlotArgs) -> None:
    """
    Plot scalar data on a single matplotlib plot.
...
def load_multi_wso_data(directories:list[str]=["experiments/runs"], prefixes:list[str]=["experiments/runs/sunflower_cost"]) -> tuple[
...
def load_single_wso_data(directories:list[str]=["experiments/runs"], prefixes:list[str]=["experiments/runs/sunflower_single"]) -> tuple[np.ndarray, np.ndarray]:
    # Get all NPZ files
    npz_files = []
    for directory in directories:
        for root, _, files in os.walk(directory):
...
def plot_bc_yield(directories:list[str]=["experiments/runs"], prefixes:list[str]=["experiments/runs/wheat_threshold_wk"]) -> tuple[np.ndarray, np.ndarray]:
    # Get all NPZ files
    npz_files = []
    for directory in directories:
        for root, _, files in os.walk(directory):
...
def find_max_in_nested_arrays(*arrays:list) -> float:
    max_value = float("-inf")
...
def counts(inds:list[int], vals:list, x:int) -> np.ndarray:
    sum = np.zeros(x + 1)
    count = np.zeros(x + 1)
...
def fix_colormap(cm:LinearSegmentedColormap) -> LinearSegmentedColormap:
...
if __name__ == "__main__":
    ...
    log_dirs = [
        ...
        "data/wheat_constrained/DQN/lnpkw-v0__rl_utils__1__1772101931",
        "data/wheat_constrained/PPO/lnpkw-v0__rl_utils__1__1772100643",
        "data/wheat_constrained/SAC/lnpkw-v0__rl_utils__1__1772101871",
        "data/wheat_constrained/RPPO/lnpkw-v0__rl_utils__1__1772101994",

        #"data/grapevine/unconstrained_baseline_2/PPO/grape-lnpkw-v0__rl_utils__1__1773304096",   # baseline_5000000
        "data/grapevine/unconstrained_LAI_v4/PPO/grape-lnpkw-v0__rl_utils__1__1775704347",
        "data/grapevine/unconstrained_LAI_v5/PPO/grape-lnpkw-v0__rl_utils__1__1775704347"
        
        # "data/grapevine/unconstrained_baseline_2/PPO/grape-lnpkw-v0__rl_utils__1__1773304096",   # baseline_5000000
        # "data/grapevine/unconstrained_TAGP_v1/PPO/grape-lnpkw-v0__rl_utils__1__1774605316",   # TAGP_v1_5000000
        # "data/grapevine/unconstrained_DVR_v1/PPO/grape-lnpkw-v0__rl_utils__1__1774605316",   # DVR_v1_5000000
        # "data/grapevine/unconstrained_GRLV_v1/PPO/grape-lnpkw-v0__rl_utils__1__1774605316",   # GRLV_v1_5000000
        # "data/grapevine/unconstrained_LAI_v3/PPO/grape-lnpkw-v0__rl_utils__1__1774427125"   # LAI_v3_5000000

        # "data/grapevine/unconstrained_baseline_2/PPO/grape-lnpkw-v0__rl_utils__1__1773304096",   # baseline_5000000
        # "data/grapevine/unconstrained_LAI_v2/PPO/grape-lnpkw-v0__rl_utils__1__1774129167",   # LAI_v2_5000000
        # "data/grapevine/unconstrained_LAI_v3/PPO/grape-lnpkw-v0__rl_utils__1__1774427125",   # LAI_v3_5000000
        # "data/grapevine/unconstrained_DVS_v1/PPO/grape-lnpkw-v0__rl_utils__1__1774427125",   # DVS_v1_5000000

        #"data/grapevine/unconstrained_baseline_2/PPO/grape-lnpkw-v0__rl_utils__1__1773304096",   # baseline_5000000
        #"data/grapevine/unconstrained_LAI_v1/PPO/grape-lnpkw-v0__rl_utils__1__1774128278",   # v1_5000000
        #"data/grapevine/unconstrained_LAI_v2/PPO/grape-lnpkw-v0__rl_utils__1__1774129167"   # v2_5000000
        #"data/grapevine/unconstrained_LAI_v2/PPO/grape-lnpkw-v0__rl_utils__1__1774423776"   # v2_1000
        
        # "data/maize/unconstrained_baseline_1/DQN/lnpkw-v0__rl_utils__1__1773847086",
        # "data/maize/unconstrained_baseline_1/PPO/lnpkw-v0__rl_utils__1__1773847086",
        # "data/maize/unconstrained_baseline_1/SAC/lnpkw-v0__rl_utils__1__1773847086",
        # "data/maize/unconstrained_baseline_1/RPPO/lnpkw-v0__rl_utils__1__1773847086"

        # "data/grapevine/unconstrained_baseline_2/DQN/grape-lnpkw-v0__rl_utils__1__1773248013",
        # "data/grapevine/unconstrained_baseline_2/PPO/grape-lnpkw-v0__rl_utils__1__1773304096",
        # "data/grapevine/unconstrained_baseline_2/SAC/grape-lnpkw-v0__rl_utils__1__1773689705",
        # "data/grapevine/unconstrained_baseline_2/RPPO/grape-lnpkw-v0__rl_utils__1__1773689705"
        ...
        #"experiments/UncontrainedControl/Grape/DQN/grape-lnpkw-v0__rl_utils__1__1749654202",
        #"experiments/UncontrainedControl/Grape/PPO/grape-lnpkw-v0__rl_utils__1__1749577592",
        #"experiments/UncontrainedControl/Grape/SAC/grape-lnpkw-v0__rl_utils__1__1749614755",
        #"experiments/UncontrainedControl/Wheat/RPPO/lnpkw-v0__rl_utils__1__1748636264",
        # "runs/grapevine/dqn/DQN/grape-lw-v0__rl_utils__1__1771502335",
        # "runs/grapevine/ppo/PPO/grape-lw-v0__rl_utils__1__1771497775",
        # "runs/grapevine/sac/SAC/grape-lw-v0__rl_utils__1__1771501866",
        # "runs/grapevine/rppo/RPPO/grape-lw-v0__rl_utils__1__1771501535"
    ]
    ...
    names = ["LAI_v4", "LAI_v5", "RPPO", "baseline"]
    data = load_scalars_from_runs(log_dirs=log_dirs, scalar_name="charts/average_reward")
    args.save_name = "data/grapevine/compare_reward_wrappers_5"
    plot_reward_figs_two(data, names, args)
    #plot_reward_figs_one(data, names, args)
    #plot_grapevine_unconstrained(data, names, args)
    ...
    #     # "data/wheat_constrained/PPO/lnpkw-v0__rl_utils__1__1772100643",
    #     # "data/wheat_constrained/SAC/lnpkw-v0__rl_utils__1__1772101871",
    #     # "data/grapevine/unconstrained_baseline/RPPO/grape-lnpkw-v0__rl_utils__1__1773136674",
    #     "data/wheat_constrained/DQN/lnpkw-v0__rl_utils__1__1772101931",
    #     "data/wheat_constrained/PPO/lnpkw-v0__rl_utils__1__1772100643",
    #     "data/wheat_constrained/SAC/lnpkw-v0__rl_utils__1__1772101871",
    #     "data/grapevine/unconstrained_baseline/RPPO/grape-lnpkw-v0__rl_utils__1__1773136674"
