...
class BaseCropModel(SimulationObject):
    ...
    **State variables:**
    ...
    def calc_rates(self, day: date, drv: WeatherDataContainer) -> None:
        ...
        CVF = 1.0 / (
            (pf.FL / params.CVL + pf.FS / params.CVS + pf.FO / params.CVO) * (1.0 - pf.FR) + pf.FR / params.CVR
        )
...
class Wofost80(BaseCropModel):
    ...
    def initialize(self, day: date, kiosk: VariableKiosk, parvalues: dict) -> None:
        ...
        self.rates = self.RateVariables(kiosk, publish=["GASS", "PGASS", "MRES", "ASRC", "DMI", "ADMI"])
...
class Wofost80Perennial(BaseCropModel):
    ...
    def calc_rates(self, day: date, drv: WeatherDataContainer) -> None:
        ...
        pf = self.part.calc_rates(day, drv)
        CVF = 1.0 / (
            (pf.FL / params.CVL(k.AGE) + pf.FS / params.CVS(k.AGE) + pf.FO / params.CVO(k.AGE)) * (1.0 - pf.FR)
            + pf.FR / params.CVR(k.AGE)
        )
        rates.DMI = CVF * rates.ASRC
        self._check_carbon_balance(day, rates.DMI, rates.GASS, rates.MRES, CVF, pf)
