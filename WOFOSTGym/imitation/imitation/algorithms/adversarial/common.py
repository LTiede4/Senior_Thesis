    ...
    def _make_disc_train_batches(
        ...
        *,
        ...
        for start in range(0, batch_size, self.demo_minibatch_size):
            ...
            obs = np.concatenate([expert_batch["obs"], gen_batch["obs"].squeeze()])
            acts = np.concatenate([expert_batch["acts"], gen_batch["acts"].squeeze()])
            next_obs = np.concatenate([expert_batch["next_obs"], gen_batch["next_obs"].squeeze()])
            dones = np.concatenate([expert_batch["dones"], gen_batch["dones"].squeeze()])
