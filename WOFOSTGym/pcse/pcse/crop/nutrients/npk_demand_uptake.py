...
class NPK_Demand_Uptake(SimulationObject):
    ...
    **External dependencies**
    ...
    class Parameters(ParamTemplate):
        ...
        NMAXRT_FR = Float(-99.0)  # maximum N concentration in roots as fraction of maximum N concentration in leaves
        PMAXRT_FR = Float(-99.0)  # maximum P concentration in roots as fraction of maximum P concentration in leaves
        KMAXRT_FR = Float(-99.0)  # maximum K concentration in roots as fraction of maximum K concentration in leaves

        NMAXST_FR = Float(-99.0)  # maximum N concentration in stems as fraction of maximum N concentration in leaves
        PMAXST_FR = Float(-99.0)  # maximum P concentration in stems as fraction of maximum P concentration in leaves
        KMAXST_FR = Float(-99.0)  # maximum K concentration in stems as fraction of maximum K concentration in leaves

        NMAXSO = Float(-99.0)  # maximum P concentration in storage organs [kg N kg-1 dry biomass]
        PMAXSO = Float(-99.0)  # maximum P concentration in storage organs [kg P kg-1 dry biomass]
        KMAXSO = Float(-99.0)  # maximum K concentration in storage organs [kg K kg-1 dry biomass]

        TCNT = Float(-99.0)  # time coefficient for N translocation to storage organs [days]
        TCPT = Float(-99.0)  # time coefficient for P translocation to storage organs [days]
        TCKT = Float(-99.0)  # time coefficient for K translocation to storage organs [days]

        NFIX_FR = Float(-99.0)  # fraction of crop nitrogen uptake by biological fixation
    ...
    class RateVariables(RatesTemplate):
        RNUPTAKELV = Float(-99.0)  # N uptake rates in organs [kg ha-1 d -1]
        RNUPTAKEST = Float(-99.0)
        RNUPTAKERT = Float(-99.0)
        RNUPTAKESO = Float(-99.0)

        RPUPTAKELV = Float(-99.0)  # P uptake rates in organs [kg ha-1 d -1]
        RPUPTAKEST = Float(-99.0)
        RPUPTAKERT = Float(-99.0)
        RPUPTAKESO = Float(-99.0)

        RKUPTAKELV = Float(-99.0)  # K uptake rates in organs [kg ha-1 d -1]
        RKUPTAKEST = Float(-99.0)
        RKUPTAKERT = Float(-99.0)
        RKUPTAKESO = Float(-99.0)

        RNUPTAKE = Float(-99.0)  # Total N uptake rates [kg ha-1 d -1]
        RPUPTAKE = Float(-99.0)  # For P
        RKUPTAKE = Float(-99.0)  # For K
        RNFIXATION = Float(-99.0)  # Total N fixated

        NDEMANDLV = Float(-99.0)  # N demand in organs [kg ha-1]
        NDEMANDST = Float(-99.0)
        NDEMANDRT = Float(-99.0)
        NDEMANDSO = Float(-99.0)

        PDEMANDLV = Float(-99.0)  # P demand in organs [kg ha-1]
        PDEMANDST = Float(-99.0)
        PDEMANDRT = Float(-99.0)
        PDEMANDSO = Float(-99.0)

        KDEMANDLV = Float(-99.0)  # K demand in organs [kg ha-1]
        KDEMANDST = Float(-99.0)
        KDEMANDRT = Float(-99.0)
        KDEMANDSO = Float(-99.0)
    ...
    def calc_rates(self, day: date, drv: WeatherDataContainer) -> None:
        ...
        # N demand [kg ha-1]
        r.NDEMANDLV = max(mc.NMAXLV * k.WLV - k.NAMOUNTLV, 0.0) + max(k.GRLV * mc.NMAXLV, 0) * delt
        r.NDEMANDST = max(mc.NMAXST * k.WST - k.NAMOUNTST, 0.0) + max(k.GRST * mc.NMAXST, 0) * delt
        r.NDEMANDRT = max(mc.NMAXRT * k.WRT - k.NAMOUNTRT, 0.0) + max(k.GRRT * mc.NMAXRT, 0) * delt
        r.NDEMANDSO = max(mc.NMAXSO * k.WSO - k.NAMOUNTSO, 0.0)

        # P demand [kg ha-1]
        r.PDEMANDLV = max(mc.PMAXLV * k.WLV - k.PAMOUNTLV, 0.0) + max(k.GRLV * mc.PMAXLV, 0) * delt
        r.PDEMANDST = max(mc.PMAXST * k.WST - k.PAMOUNTST, 0.0) + max(k.GRST * mc.PMAXST, 0) * delt
        r.PDEMANDRT = max(mc.PMAXRT * k.WRT - k.PAMOUNTRT, 0.0) + max(k.GRRT * mc.PMAXRT, 0) * delt
        r.PDEMANDSO = max(mc.PMAXSO * k.WSO - k.PAMOUNTSO, 0.0)

        # K demand [kg ha-1]
        r.KDEMANDLV = max(mc.KMAXLV * k.WLV - k.KAMOUNTLV, 0.0) + max(k.GRLV * mc.KMAXLV, 0) * delt
        r.KDEMANDST = max(mc.KMAXST * k.WST - k.KAMOUNTST, 0.0) + max(k.GRST * mc.KMAXST, 0) * delt
        r.KDEMANDRT = max(mc.KMAXRT * k.WRT - k.KAMOUNTRT, 0.0) + max(k.GRRT * mc.KMAXRT, 0) * delt
        r.KDEMANDSO = max(mc.KMAXSO * k.WSO - k.KAMOUNTSO, 0.0)
        ...
        r.RNFIXATION = max(0.0, p.NFIX_FR * r.NDEMAND) * NutrientLIMIT

        # NPK uptake rate from soil
        if k.DVS < p.DVS_NPK_STOP:
            r.RNUPTAKE = max(0.0, min(r.NDEMAND - r.RNFIXATION, k.NAVAIL, p.RNUPTAKEMAX)) * NutrientLIMIT
            r.RPUPTAKE = max(0.0, min(r.PDEMAND, k.PAVAIL, p.RPUPTAKEMAX)) * NutrientLIMIT
            r.RKUPTAKE = max(0.0, min(r.KDEMAND, k.KAVAIL, p.RKUPTAKEMAX)) * NutrientLIMIT
        else:
        ...
        if r.NDEMAND == 0.0:
            r.RNUPTAKELV = r.RNUPTAKEST = r.RNUPTAKERT = 0.0
        else:
        ...
        if r.PDEMAND == 0.0:
            r.RPUPTAKELV = r.RPUPTAKEST = r.RPUPTAKERT = 0.0
        else:
        ...
        if r.KDEMAND == 0.0:
            r.RKUPTAKELV = r.RKUPTAKEST = r.RKUPTAKERT = 0.0
        else:
    ...
    def reset(self) -> None:
        ...
        r.RNUPTAKELV = r.RNUPTAKEST = r.RNUPTAKERT = r.RNUPTAKESO = r.RPUPTAKELV = r.RPUPTAKEST = r.RPUPTAKERT = (
            r.RPUPTAKESO
        ) = r.RKUPTAKELV = r.RKUPTAKEST = r.RKUPTAKERT = r.RKUPTAKESO = r.RNUPTAKE = r.RPUPTAKE = r.RKUPTAKE = (
            r.RNFIXATION
        ) = r.NDEMANDLV = r.NDEMANDST = r.NDEMANDRT = r.NDEMANDSO = r.PDEMANDLV = r.PDEMANDST = r.PDEMANDRT = (
            r.PDEMANDSO
        ) = r.KDEMANDLV = r.KDEMANDST = r.KDEMANDRT = r.KDEMANDSO = r.NDEMAND = r.PDEMAND = r.KDEMAND = 0
